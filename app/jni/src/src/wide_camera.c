...
int WideCamera_IsMapMenu(int module, int submodule) {
  // Dungeon map, world map, and the flute's destination picker. Their sprites
  // are projected in map-screen coordinates rather than the gameplay camera, so
  // applying the camera delta would shift them out of place.
...
int WideCamera_ClampToBounds(int logical_x, int left_bound,
  ...
  // If the area is too narrow for a margin on each side, don't lock at all.
...
int WideCamera_InterpolateTransition(int logical_x, int start_logical_x,
  ...
  int offset = start_offset + RoundedDivide(offset_delta * progress, distance);
